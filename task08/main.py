...
class HelloWorld(mglw.WindowConfig):
    ...
    def render(self, time: int, frame_time):
        ...
        for i_bone in range(num_bone):
            ...
            bone2globalTransformation[i_bone] = (
                bone2globalTransformation[i_bone_parent] @ bone2relativeTransformation[i_bone]
            )

        for i_vtx in range(self.vtx2xyz_ini.shape[0]):  # for each point in mesh
            p0: npt.NDArray[np.float32] = self.vtx2xyz_ini[i_vtx]
            p0: npt.NDArray[np.float32] = np.append(
                p0, np.array([1.0])
            ...
            for idx in range(4):  # in gltf each vertex is associated with four bones
                ...
                p1 += globalTransformation @ inverseBindingMatrix @ p0 * w
